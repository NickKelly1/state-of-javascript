...
function VerifyEmailPage(props: IVerifyEmailPageProps) {
  ...
  const handleSuccess = useCallback((result: VerifyEmailMutation) => {
  ...
  const [doSubmit, submitState] = useMutation<VerifyEmailMutation, ApiException>(
    async () => {
      const result = await api.verifyEmail({ token: router.query['token'] as $DANGER<string> });
      return result;
    }, {
